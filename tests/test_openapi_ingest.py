...
from services.processor import (
    WikiProcessor, _apis_from_openapi, _parse_frontmatter, _readme_summary,
)
...
_SPEC = {
    "openapi": "3.1.0",
    "paths": {
        "/payments/charge": {"post": {"summary": "Charge a saved credit card",
...
def test_parse_frontmatter():
    fm = _parse_frontmatter({"r.md": "---\ntype: how-to\nsource_app: kb\ntags: [a, b-c]\n---\n# X\nbody"})
    assert fm["type"] == "how-to" and fm["source_app"] == "kb" and fm["tags"] == ["a", "b-c"]
    assert _parse_frontmatter({"r.md": "# no frontmatter"}) == {}
