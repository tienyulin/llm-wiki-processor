...
class WikiProcessor:
    ...
    async def _update_wiki_for_app(
        ...
        # Merge: keep files from other apps, update files from this app
        merged_wiki = {
            **{path: content for path, content in current_wiki.items()
               if self._extract_source_app(content) != source_app},
            **app_wiki_files,
        }
    ...
    def _log_audit(self, source_app: str, files_count: int, status: str, files_updated: list):
        ...
        new_content = existing.rstrip() + "\n" + __import__("json").dumps(log_entry) if existing else __import__("json").dumps(log_entry)
        ...
        try:
            ...
            if is_first_run:
                ...
                files_updated = [path for path in wiki.keys()
                               if self._extract_source_app(wiki[path]) == source_app]
            else:
